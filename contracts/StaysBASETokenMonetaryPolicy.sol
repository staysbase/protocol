...
/**
 * @title StaysBASEToken Monetary Supply Policy
 * @dev This is an implementation of the StaysBASEToken Index Fund protocol.
 *      StaysBASEToken operates symmetrically on expansion and contraction. It will both split and
 *      combine coins to maintain a stable unit price.
 *
 *      This component regulates the token supply of the StaysBASEToken ERC20 token in response to
 *      market oracles.
 */
contract StaysBASETokenMonetaryPolicy is OwnableUpgradeSafe {
    ...
    function setBASEToken(address _BASE)
        ...
        onlyOwner
    {
        StaysBASE = StaysBASEToken(_BASE);
    }
    ...
    function rebase() external {
        ...
        if (supplyDelta > 0 && StaysBASE.totalSupply().add(uint256(supplyDelta)) > MAX_SUPPLY) {
            supplyDelta = (MAX_SUPPLY.sub(StaysBASE.totalSupply())).toInt256Safe();
        }

        uint256 nextSupply = StaysBASE.rebase(epoch, supplyDelta);
     ...
     */
    function initialize(StaysBASEToken BASE_)
        public
        initializer
    {
        __Ownable_init();
        ...
        returns (int256)
    {
        ...
        return StaysBASE.totalSupply().toInt256Safe()
