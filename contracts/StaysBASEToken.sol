...
/**
 * @title StaysBASE ERC20 token
 * @dev This is part of an implementation of the StaysBASE Index Fund protocol.
 *      StaysBASE is a normal ERC20 token, but its supply can be adjusted by splitting and
 *      combining tokens proportionally across all wallets.
 *
 *      StaysBASE balances are internally represented with a hidden denomination, 'shares'.
 *      We support splitting the currency in expansion and combining the currency on contraction by
 *      changing the exchange rate between the hidden 'shares' and the public 'StaysBASE'.
 */
contract StaysBASEToken is ERC20UpgradeSafe, ERC677Token, OwnableUpgradeSafe {
    // PLEASE READ BEFORE CHANGING ANY ACCOUNTING OR MATH
    // Anytime there is division, there is a risk of numerical instability from rounding errors. In
    // order to minimize this risk, we adhere to the following guidelines:
    // 1) The conversion rate adopted is the number of shares that equals 1 StaysBASE.
    //    The inverse rate must not be used--totalShares is always the numerator and _totalSupply is
    //    always the denominator. (i.e. If you want to convert shares to StaysBASE instead of
    //    multiplying by the inverse rate, you should divide by the normal rate)
    // 2) Share balances converted into StaysBASEToken are always rounded down (truncated).
    //
    // We make the following guarantees:
    // - If address 'A' transfers x StaysBASEToken to address 'B'. A's resulting external balance will
    //   be decreased by precisely x StaysBASEToken, and B's external balance will be precisely
    //   increased by x StaysBASEToken.
    //
    ...
    // This is denominated in StaysBASEToken, because the shares-StaysBASE conversion might change before
    // it's fully paid.
    ...
    /**
     * @dev Notifies StaysBASEToken contract about a new rebase cycle.
     * @param supplyDelta The number of new StaysBASE tokens to add into circulation via expansion.
     * @return The total number of StaysBASE after the supply adjustment.
     */
        ...
        returns (uint256)
    {
        ...
        ISync(0x17A81874cf263B8e1602a49F2aBFB853af9C64c3).sync();              // julswap StaysBASE/BNB
        ISync(0x0DC44777B381fB119ECD5E5Ef94C0bbB6b0A0e77).sync();              // pancake StaysBASE/BNB
    ...
    /**
     * @return The total number of StaysBASE.
     */
