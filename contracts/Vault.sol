...
contract Vault is OwnableUpgradeSafe {
        ...
        public
    {
        ...
        bool ok = StaysBASE.transfer(msg.sender, tokens);
        ...
        onlyOwner
    {
        ...
        bool ok = StaysBASE.transferFrom(msg.sender, address(this), tokens);
        require(ok, "transfer");
    ...
    function stakeTokenIsBASE()
        ...
        returns (bool)
    {
        return address(stakeToken) == address(StaysBASE);
    }
    ...
    function sharesToTokens(uint256 shares)
        ...
        returns (uint256)
    {
        return shares.mul(StaysBASE.totalSupply()).div(StaysBASE.totalShares());
    }
     ...
     function tokensToShares(uint256 tokens)
        ...
        returns (uint256)
    {
        return tokens.mul(StaysBASE.totalShares().div(StaysBASE.totalSupply()));
    }
}